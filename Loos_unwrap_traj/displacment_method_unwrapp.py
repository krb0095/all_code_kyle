...
if __name__ in '__main__':
    ...
    for frame in traj:
        ...
        if traj.index() == 0:
            # we have no need to update the unwrapped postions of the seelctions center of mass
            xi = np.array([[lip.centerOfMass()[0],lip.centerOfMass()[1],lip.centerOfMass()[2]] for lip in lipids])
            ...
            # deifne the new xiu1 
            xi1 = np.array([[lip.centerOfMass()[0],lip.centerOfMass()[1],lip.centerOfMass()[2]]for lip in lipids])
            # find x_ui1 
            ...
            # CODE For loos to make a pdb  and traj 
            if args.output_traj:
                output.periodicBox(loos.GCoord(1000000, 1000000, 1000000))
                frame_copy = lipid_sel.copy()
                # the postion in the wrapped traj is xi1 
                # we are moving to xui1 
                # create a vector to store each selection postion change for translation
                translation_vector = xui1 - xi1 # each vector will tell us how to move the lipid
                for vec,lip in zip(translation_vector,lipids):
                    trans_vec = loos.GCoord(vec[0],vec[1],vec[2])
                    lip.translate(trans_vec)
                    output.append(lip)
                outtraj.writeFrame(output)
